...
```javascript
SeS('SomeJavaSwingTree').DoClickNode(path, separator, pathType, clickType, xOffset, yOffset)
```
...
```javascript
SeS('SomeJavaSwingTree').DoCollapse(path, separator, pathType)
```
...
```javascript
SeS('SomeJavaSwingTree').DoExpand(path, separator, pathType)
```
