...
```javascript
SeS('SomeUltraAppMenu').DoDumpWidget(dumpProperties, maxDepth, trimValue, indent, filePath, append, ignoreProps)
```
...
```javascript
SeS('SomeUltraAppMenu').DoFindByName(textOrRegexp, depth)
```
...
```javascript
SeS('SomeUltraAppMenu').DoFindByText(textOrRegexp, depth)
```
...
```javascript
SeS('SomeUltraAppMenu').DoGetWidgetProperty(name)
```
