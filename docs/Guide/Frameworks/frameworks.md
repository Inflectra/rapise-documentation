...
1. [Grouping of test cases](#grouping_of_test_cases). Test cases can be organized in subfolders and each test case can have separate RVL Sheets and functions.
...
5. [Refactoring](#refactoring). Moving, renaming, cloning, importing and exporting test cases is necessary to easily organize the framework as it grows and develops.
6. [Cloning of the entire testing framework](#cloning-of-the-entire-test-framework). Ability to clone and save the framework as a whole, preserving settings for test management - test sets and configurations.
8. [Parametrization](#parametrization), including runtime settings (intervals, screenshots), and configuration settings (initial URL, startup paths, etc.).
9. An elaborate [framework development workflow](#framework-development-workflow). As a rule, it is impossible to foresee everything at the beginning of development.  It also takes time to learn all the features of Rapise. That said, the first test cases should appear as early as possible. A good workflow allows you to get started quickly, without having to fully explore all the features and without sacrificing the quality of the framework in the future.
10. A [navigation system](#navigation-system) is necessary to navigate a framework with hundreds or thousands of test cases and dozens of configurations. Moreover, the capabilities for internal documentation of individual test cases, functions, and Page Objects are important.
11. [Integration with test management](#integration-with-test-management). Test sets, night runs, work with flaky tests, launch history.
12. [Teamwork and distributed development](#teamwork-and-distributed-development). Distributed development and test launches where different participants have different levels of knowledge and expertise. If you're developing a medical system, for example, someone will know medicine better, and someone will know test automation better. And it is important to ensure harmonious interaction between the different members of the team.
13. [Maintenance](#maintenance). testing frameworks are developed for the long term, which means that they must be constantly maintained. It is perfectly normal for a test to fail with minimal changes to the application, and hence the convenience of constantly fixing them comes to the forefront.
14. [Mixed Mode Tests](#mixed-mode-tests). Integration tests often interact with several different types of applications. There's the mobile client, the web app, the desktop, and the API. And the usability of interaction with all of these applications comes to the force.
