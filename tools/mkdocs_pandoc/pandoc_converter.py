...
class PandocConverter:
    ...
    def flatten_pages(self, pages, level=0, parent_topic_name=''):
        ...
        for page in pages:
            ...
            # Make next embedded topic higher level (if this one is a group topic and next)
            ...
            if type(page) is dict:
                if type(list(page.values())[0]) is str:
                    ...
                    if len(parent_topic_name)>0:
                        if list(page.keys())[0]==parent_topic_name:
                            reslevel = level -1
                        elif level>1:
                            flattened.append(
                                    {
                                        'title': parent_topic_name,
                                        'file': 'emptyfile.md',
                                        'level': level-1,
                                    })
                            reslevel = level
    ...
    def convert(self):
        ...
        for page in pages:
            ...
            # Tanur: Skip mkdocs.yml line with URL instead of file path.
            if 'file' in page and page['file'].startswith("http:") :
                continue
